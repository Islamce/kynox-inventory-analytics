  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <button type="button" className="bg-sky-700 hover:bg-sky-800 text-white rounded-lg px-4 py-1.5 text-sm" onClick={() => setParams({ material: input.trim() })}>
          Open profile
        </button>
      </div>
      ...
      {data && (
        <>
          <Card title={`${data.material} — ${data.stock.description ?? 'no description'}`}>
            ...
            {data.xyz && <p className="text-xs text-slate-500 mt-2">XYZ reasoning: {data.xyz.reason}</p>}
            {data.movementCategory && <p className="text-xs text-slate-500">Movement category: {data.movementCategory.reason}</p>}
          ...
          {data.forecast && !data.forecast.insufficientData && data.forecast.forecast && (
            <Card title={`Forecast (${data.forecast.best})`} subtitle={data.forecast.note}>
              ...
              <details className="text-sm mt-2">
                <summary className="cursor-pointer text-sky-700">Method comparison (holdout back-test)</summary>
                <table className="mt-2 text-sm w-full max-w-md">
                  <thead><tr className="text-left text-slate-500"><th>Method</th><th className="text-right">WAPE %</th><th className="text-right">MAE</th></tr></thead>
                  <tbody>
          ...
          {data.forecast?.insufficientData && (
            <Card title="Forecast"><p className="text-sm text-slate-600">{data.forecast.note}</p></Card>
          )}
          ...
          {data.planning && (
            <Card title="Planning proposal" subtitle="Recommendations requiring planner review — never applied automatically">
              {data.planning.insufficientData
                ? <p className="text-sm text-slate-600">{data.planning.note}</p>
                : data.planning.proposed && (
                  <div className="text-sm space-y-2">
                    <div className="grid md:grid-cols-3 gap-3">
                      <div className="bg-slate-50 rounded-lg p-3">
                        <p className="text-xs text-slate-500">Proposed safety stock</p>
                        <p className="text-lg font-bold tabular-nums">{data.planning.proposed.safetyStock.safetyStock.toLocaleString()}</p>
                        <p className="text-xs text-slate-400">{data.planning.proposed.safetyStock.formula}</p>
                        <p className="text-xs text-slate-400">current: {data.planning.current?.safetyStock ?? '—'}</p>
                      </div>
                      <div className="bg-slate-50 rounded-lg p-3">
                        <p className="text-xs text-slate-500">Proposed reorder point</p>
                        <p className="text-lg font-bold tabular-nums">{data.planning.proposed.reorderPoint.reorderPoint.toLocaleString()}</p>
                        <p className="text-xs text-slate-400">{data.planning.proposed.reorderPoint.formula}</p>
                        <p className="text-xs text-slate-400">current: {data.planning.current?.reorderPoint ?? '—'}</p>
                      </div>
                      <div className="bg-slate-50 rounded-lg p-3">
                        <p className="text-xs text-slate-500">Proposed min / max</p>
                        ...
                        <p className="text-xs text-slate-400">{data.planning.proposed.minMax.formula}</p>
                      </div>
                    </div>
